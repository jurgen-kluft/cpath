...
namespace ncore
{
    filepath_t::filepath_t() : m_dirpath(), m_filename(pathreg_t::sNilStr), m_extension(pathreg_t::sNilStr) {}
    filepath_t::filepath_t(const filepath_t& other) : m_dirpath(other.m_dirpath)
    {
        m_filename  = m_dirpath.m_device->m_root->m_pathdb->attach(other.m_filename);
        m_extension = m_dirpath.m_device->m_root->m_pathdb->attach(other.m_extension);
    }

    filepath_t::filepath_t(pathstr_t* filename, pathstr_t* extension) : m_dirpath()
    {
        m_filename  = m_dirpath.m_device->m_root->m_pathdb->attach(filename);
        m_extension = m_dirpath.m_device->m_root->m_pathdb->attach(extension);
    }
    ...
    {
        pathreg_t* root = m_dirpath.m_device->m_root;
    ...
    void filepath_t::clear()
    {
        ...
        root->release_filename(m_filename);
        root->release_extension(m_extension);
        m_filename  = pathreg_t::sNilStr;
        m_extension = pathreg_t::sNilStr;
    }

    bool filepath_t::isRooted() const { return m_dirpath.isRooted(); }
    bool filepath_t::isEmpty() const { return m_dirpath.isEmpty() && m_filename == pathreg_t::sNilStr && m_extension == pathreg_t::sNilStr; }
    ...
    void filepath_t::setFilename(crunes_t const& filenamestr)
    {
        ...
        root->release_filename(m_filename);
        root->release_extension(m_extension);
        m_filename  = m_dirpath.m_device->m_root->m_pathdb->attach(out_filename);
        m_extension = m_dirpath.m_device->m_root->m_pathdb->attach(out_extension);
    }
    ...
    void filepath_t::split(s32 pivot, dirpath_t& left, filepath_t& right) const
    {
        ...
        // right.m_filename = m_dirpath.m_device->m_root->m_pathdb->attach(m_filename);
        // right.m_extension = m_dirpath.m_device->m_root->m_pathdb->attach(m_extension);
    }
    ...
    void filepath_t::truncate(filepath_t& left, pathnode_t*& folder) const
    {
        ...
        // left.m_filename = m_dirpath.m_device->m_root->m_pathdb->attach(m_filename);
        // left.m_extension = m_dirpath.m_device->m_root->m_pathdb->attach(m_extension);
    }
    ...
    void filepath_t::truncate(pathnode_t*& folder, filepath_t& filepath) const
    {
        ...
        // filepath.m_filename = m_dirpath.m_device->m_root->m_pathdb->attach(m_filename);
        // filepath.m_extension = m_dirpath.m_device->m_root->m_pathdb->attach(m_extension);
    }
    ...
    void filepath_t::to_string(runes_t& str) const
    {
        ...
        crunes_t filenamestr(m_filename->str(), 0, m_filename->m_len, m_filename->m_len);
        nrunes::concatenate(str, filenamestr);
        crunes_t extensionstr(m_extension->str(), 0, m_extension->m_len, m_extension->m_len);
        nrunes::concatenate(str, extensionstr);
    }

    s32 filepath_t::to_strlen() const
    {
        s32 len = m_dirpath.to_strlen();
        len += m_filename->m_len;
        len += m_extension->m_len;
        return len;
    }

    s32 filepath_t::compare(const filepath_t& right) const
    {
        s32 const fe = m_dirpath.m_device->m_root->m_pathdb->compare(m_filename, right.m_filename);
        if (fe != 0)
            return fe;
        s32 const ce = m_dirpath.m_device->m_root->m_pathdb->compare(m_extension, right.m_extension);
        if (ce != 0)
            return ce;
    ...
    filepath_t& filepath_t::operator=(const filepath_t& fp)
    {
        ...
        root->release_filename(m_filename);
        root->release_extension(m_extension);
        m_filename  = fp.m_dirpath.m_device->m_root->m_pathdb->attach(m_filename);
        m_extension = fp.m_dirpath.m_device->m_root->m_pathdb->attach(m_extension);
        return *this;
    }
